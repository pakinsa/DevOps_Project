...
#### For a specific CloudFront Origin Access Control (OAC) setup like ours:

Since your app.js has app.use(cors()); (with no arguments), it is currently set to "Allow All Origins."
...
We need Use StringEquals if you want to ensure the request comes from that one specific distribution and nothing else.
...
And one must create a new invalidation for /* every single time we change our code and sync to S3, this is in case we took 
