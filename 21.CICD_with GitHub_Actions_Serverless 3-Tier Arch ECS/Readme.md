...
#### ADD NEW RATING Logic to FRONTEND & BACKEND
#### Lets tamper with the code, and do software development tasks here

The controller.js files are the point where our code meets the database for this app. Very crucial point to note. And since we havent used Prisma in our code routes but only in the right and necessary controllers... any new feature too will have to be meet the database at the controllers not routes. You shouldn't add Prisma to the routes if your app uses a Controller pattern. Instead, you add the logic to the Controller and the entry point to the Route. And since our project structure uses Controllers, we should keep it clean by adding the logic to **BooksController.js** and referencing it in our *index.js**. 


* ***2ways to add new feature logic:***

* ***ORM-Way***
Even though we haven't used Prisma in our routes yet, and since we've used it in our project for data migration already, meaning we could use Prisma in the BookController.js for this new feature. 

This will upgrade our app from old to modern ORM application. Since we already have the schema.prisma file, the backend code becomes "Type-Safe." If we try to save a string into a number column, the code will throw an error before it even hits the database. Security mindset here to avoid SQL-injections.

Here is the BookController.js code below that integrates Prisma, which we could use:

Routes (index.js): Only defines the "URL" (e.g., /api/books/:id/rating).
Controller (BooksController.js): Contains the "Action" (The Prisma code that talks to RDS).
...
exports.updateRating = async (req, res) => {
  ...
  try {
    ...
    res.status(200).json({ message: "Rating updated!", updatedBook });
  } catch (error) {
    res.status(500).json({ error: "Failed to update rating in RDS" });
  }
};
```

* ***Old-SQL-Way***
But we want to stay consistent with the old original logic structure of the code/app development by maintaining SQL Query for the new feature, but hope we don't have a typo error.

Routes (index.js): Only defines the "URL" (e.g., /api/books/:id/rating).
Controller (BooksController.js): Contains the "Action" (The SQL code that talks to RDS).

Though there are other patterns of writing frontend and backend logic apart from this, below are those patterns:


- ***Common Patterns for Frontend & Backend Logic***

There are three main ways developers structure this logic. Your app currently uses the Standard MVC/Controller pattern:

| Pattern | Frontend Logic | Backend Logic |
| :--- | :--- | :--- |
| **Standard (Your App)** | Component calls a named API route using `fetch` or `axios`. | Routes pass the request to a Controller, which uses `Prisma`/`SQL`. |
| **Monolith (Old School)** | `HTML` is rendered on the server (`EJS`/`Pug`). No `fetch` needed. | The server handles the form post and redirects the page. |
| **State Management** | Uses `Redux` or `React Query` to sync the UI with the `DB` automatically. | Usually follows the same Controller pattern but with more complex "Services." |
...
File to Edit: backend/controllers/BooksController.js
Add a new function called updateRating at the bottom of the file.
...
exports.updateRating = (req, res) => {
  ...
  const sql = "UPDATE book SET rating = ? WHERE id = ?";
...
File to Edit: backend/routes/index.js
Add the new route that points to the controller function you just created.

```javascript
// ... existing routes ...
router.put('/books/:id/rating', booksController.updateRating); // Add this line
...
Add the state, the submit function, and the UI.

1. Inside the App() function:
```tsx
const handleRating = async (id: number, value: string) => {
  await fetch(`${API_URL}/api/books/${id}/rating`, {
    method: 'PUT',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ rating: value }),
  });
  alert(`Book ${id} updated!`);
  fetchBooks(); // Refresh charts
};
```


2. In the return block (above your charts):
```tsx
<div className="p-4 bg-white text-black">
  <h2 className="font-bold">Rate Your Books</h2>
  {books.map((book) => (
    <div key={book.id} className="flex justify-between border-b p-2">
      ...
      <div>
        <input 
          ...
          onBlur={(e) => handleRating(book.id, e.target.value)} 
        />
        <span>Current: {book.rating || 0}</span>
      </div>
    </div>
  ))}
</div>
...
DevOps Checklist
Dependencies: Ensure @prisma/client is in your package.json for the backend.
Dockerfile: Since you are using Prisma in the code now, your Dockerfile must run npx prisma generate after npm install but before the app starts, or the ECS task will crash.
RDS Column: Since you already added the rating column via the jump server, this code will map to it perfectly.
...
Since you are a DevOps engineer and didn't write the code, the easiest way to handle this is to treat the Rating as just another field in the data flow.
Because AddEditBookModal.tsx is likely a separate file in your src/components/ folder, that is where the actual "Textfield" needs to be added. However, we must first update the Data Model so the TypeScript compiler doesn't throw errors during your CI/CD build.

* **Step 1: Update the Model**
Find the file src/models/Books.ts. You need to add rating to the interfaces so the app knows books now have this property.
...
* **Step 2a: The "Textfield" in the Modal**
Look in your project folder for src/components/AddEditBookModal.tsx. Inside that file, you will likely see Ant Design (antd) components. You need to insert a new <Form.Item> block:
...
But looks like the src/components/AddEditBookModal.tsx file is just a "wrapper." The actual text fields (Title, Description, etc.) are inside a sub-component called BookCreateEditForm.tsx. To get that "Rating" text field to show up, you need to find and edit src/components/BookCreateEditForm.tsx. Look for the <Form> section. You will see several <Form.Item> blocks. You need to copy one and modify it for the rating or paste the code above inside.



* **Step 2b: Add Rating to View in the Modal**
Add Rating in the ```ViewBookModal.tsx``` so that frontend can display newly added rating when requested.





* **Step 3:** Add b.rating and rating to get function in BooksController.js
* Add `b.rating=rating`, and 
* Update `get` function in ```BooksController.js```
...
In your ```BooksController.js```, update the create and update P functions to catch that new field from the request body:

* **For Create:**
```javascript
const { title, description, releaseDate, pages, author: authorId, rating } = req.body;

db.query('INSERT INTO book (title, releaseDate, description, pages, authorId, rating, createdAt, updatedAt) VALUES (?, ?, ?, ?, ?, ?, ?, ?)', 
[title, new Date(releaseDate), description, pages, authorId, rating, new Date(), new Date()], ...)
```

- For Update:
```javascript
db.query('UPDATE book SET title = ?, releaseDate = ?, description = ?, pages = ?, authorId = ?, rating = ?, updatedAt = CURRENT_TIMESTAMP WHERE id = ?', 
[title, new Date(releaseDate), description, pages, authorId, rating, bookId], ...)
```




The Flow of Data (The "Wiring"):
Frontend: You add the field in BookCreateEditForm.tsx.
Frontend: When you click "Add", AddEditBookModal.tsx calls formInstance.validateFields(). Because you updated the BookFormDTO interface earlier, it will now include the rating value in the values object.
Backend: That object is sent to your ECS Fargate container.
Backend: Your BooksController.js (which you updated with req.body.rating) picks it up and sends the SQL INSERT or UPDATE to RDS.




![alt text](img/4a.Books_ts.PNG) 

![alt text](img/4b.editmodal_tsx.PNG) 

![alt text](img/4c.function_create_rating.PNG) 

![alt text](img/4d.function_update_rating.PNG) 

![alt text](img/4d.view_modal_rating.PNG) 

![alt text](img/4e.backend_push_rating.PNG) 

![alt text](img/4f.frontend_push_rating.PNG) 

![alt text](img/4g.5stars.PNG) 

![alt text](img/5a.Rating_Sucss.PNG)




We had errors of deleting authors without deleting books first, but that was how the developer 
wrote the code.

We want the error to display nicely to the user without json. We had to edit ```AuthorsController.js``` file 


In ```AuthorsController.js``` we replaced the delete function with this:
...
This change stopped the shutting down of our fargate tasks, and display the right info
to the user.


![alt text](img/6a.deprovisioning_at_error_01.PNG) 

![alt text](img/6a.deprovisioning_at_error_02.PNG) 

![alt text](img/6c.error_handled_no_crash.PNG)





FINALLY: DEPLOYMENT STRATEGY

While Rolling Update is the default strategy for Amazon ECS on AWS Fargate, several other strategies allow for more control over how traffic moves to your new code. 

*1. Rolling Update (The Default)*
What it is: ECS gradually replaces old tasks with new ones. It starts new tasks, waits for them to pass health checks, and then stops the old ones.
Best for: Simple updates where you want zero downtime but don't need a perfectly isolated testing environment before switching.
Control: You define minimumHealthyPercent (how many tasks must stay "up" during the swap) and maximumPercent (how many extra tasks can run temporarily). 



 
*2. Blue/Green Deployment (via AWS CodeDeploy)* 
What it is: You run two identical environments: Blue (current production) and Green (new version).
How it works in Fargate:
Fargate spins up the "Green" tasks in a separate target group.
You can test the Green version using a "test listener" (e.g., port 8080) before anyone else sees it.
Once verified, AWS CodeDeploy flips the Application Load Balancer to point all production traffic to Green.
Best for: Critical apps where you need an instant rollback button. 


*3. Canary Deployment*
What it is: A specific type of Blue/Green deployment where you shift only a small percentage of traffic (e.g., 10%) to the new version first.
How it works in Fargate: Managed through CodeDeploy, you can set "Linear" or "Canary" configurations (e.g., CodeDeployDefault.ECSCanary10Percent5Minutes) to slowly ramp up traffic.
Best for: Testing new features on real users in small batches to monitor for bugs or performance issues before a full rollout. 



*4. Recreate Strategy*
What it is: All old tasks are stopped before any new tasks are started.
Best for: Situations where your application cannot have two versions running at the same time (e.g., singleton processes or strict database locks).
Downside: It causes downtime because there is a gap where no tasks are running. 

 
**Summary Comparison**

| Strategy | Zero Downtime? | Rollback Speed | Extra Infrastructure Cost |
| :--- | :--- | :--- | :--- |
| **Rolling** | Yes | Minutes | Low |
| **Blue/Green** | Yes | Seconds | High (temporarily doubles tasks) |
| **Canary** | Yes | Seconds | High (temporarily doubles tasks) |
| **Recreate** | No | Slow | None |
